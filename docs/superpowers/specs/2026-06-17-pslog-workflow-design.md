...
- 스킬의 **물리적 배포 방식**(플러그인/마켓플레이스/pslog 가 내용 제공) 확정 — §9 후속.
...
1. **스킬 배포 방식**: `pslog-workflow` 가 각 프로젝트에 물리적으로 어떻게 설치되나
   (플러그인/마켓플레이스 vs pslog 가 내용 API 제공 vs 수동 복사 1회). 범용성·업데이트 전파에 영향.
2. **"코드 들어옴" 판정 강도**(§7.2): push 의 코드파일 변경 유무만 볼지, 브랜치-task 매칭을 어디까지 신뢰할지.
   v1 은 가벼운 쪽(브랜치명 `feat/task-NNN-*` + 코드 변경 존재) 권장.
3. **brief/spec/plan "있음" 판정**: 파일 존재만으로 충분한지 vs 최소 섹션(DoD 등) 채움까지 볼지.
   v1 은 파일 존재 + 비어있지 않음(가벼운 쪽) 권장.
4. **게이트 백필 시 PLAN.md 자동 수정**의 사용자 승인 흐름(자동 vs 제안 후 확인).
