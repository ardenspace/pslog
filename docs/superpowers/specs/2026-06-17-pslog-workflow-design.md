...
### 4.5 자동 강제 — SessionStart hook
CLAUDE.md 트리거 한 줄(§4.3)은 프로젝트마다 수동이다. 이를 없애기 위해 플러그인에 **SessionStart hook** 을 함께 배포한다.
- 설치된 프로젝트에서 세션 시작 시, hook 이 **pslog 관리 프로젝트인지 판별**(repo 루트에 `PLAN.md` 존재 등)하고,
  맞으면 "이 프로젝트는 pslog-workflow 의식을 따른다 — 작업 시작/코드 진입 시 스킬 호출" 트리거를 context 에 주입한다.
- pslog 프로젝트가 아니면 아무것도 주입하지 않음(다른 프로젝트에서 오작동 방지).
- 효과: **설치만 하면** 어느 pslog 프로젝트서든 CLAUDE.md 손 안 대고 자동 강제 → "어느 프로젝트나 설치=끝".
- read-only 원칙과 무관 — hook 은 context 주입만, repo 파일 수정 없음.
- 크로스플랫폼: hook 스크립트는 Windows 포함 동작해야 하므로 node 기반 권장(`${CLAUDE_PLUGIN_ROOT}` 로 경로 참조).
...
- 자동 강제: 플러그인에 **SessionStart hook** 배포 — pslog 프로젝트(PLAN.md 존재)면 세션 시작 시 트리거 자동 주입,
  CLAUDE.md 수동 편집 불필요. node 기반(Windows 호환). ✅
