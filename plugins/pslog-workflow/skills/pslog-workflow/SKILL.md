...
4. **준비 문서 작성** — `references/templates.md` 의 템플릿으로:
   ...
   - heavy → `docs/tasks/task-NNN/spec.md` 작성 → **사용자 승인** → `plan.md` 작성 →
     **사용자 승인** → 코드.
5. **구현** — plan 의 Step 을 하나씩 따라 코드. **각 Step 끝엔 build/typecheck + 기존 회귀 스위트 실행으로 안 깨졌나만 확인**한다
   (기존 회귀 스위트 = 이전 task들이 6단계에서 남긴 테스트 — 이번 step 이 *과거 동작*을 깼으면 여기서 바로 잡는다.
   *새* 테스트 작성·코드 리뷰는 step 단위로 *안 한다* — 응집·일관성은 퍼즐이 다 맞춰진 뒤에만 보인다. 나무 단위로 쪼개면 숲을 못 봄.
   이번 task 의 테스트는 생략이 아니라 **끝 검증으로 이연**된 것 — 6단계에서 반드시 되돌아온다).
...
6. **끝 검증** — 전체 구현이 끝나면 → **독립 리뷰어 서브에이전트 리뷰 + 수정 → e2e(heavy) 또는 brief DoD 검증 명령(light)** 통과.
   ...
   **세션 히스토리 없는 리뷰어 서브에이전트**에게 diff *파일* + 준비 문서 + 계약/DoD 렌즈만 주고 시킨다
   ...
   e2e/DoD 통과 후 **회귀 테스트 남기기**: DoD·T1~Tn 중 자동화 가능한 것을 테스트 스위트에 커밋한다
   — 다음 task 가 이 동작을 깨면 검증 명령이 잡아줘야 "남긴 것"이다. 한 번 통과하고 휘발되는 검증만으론 이 게이트를 못 지난다.
   자동화 가능한 검증이 하나도 없으면 그 이유를 handoff `### 결정`에 1줄 기록.
...
| 구현 → 마무리 | 끝 검증 통과 확인 (리뷰 + e2e/DoD 명령 + 회귀 테스트 남김) |
