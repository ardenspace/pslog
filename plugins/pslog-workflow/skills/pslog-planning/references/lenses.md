...
### 렌즈 3 — 설계·결정 (Eng)
질문: 핵심 아키텍처? 기존 인프라 재사용? 아래 **실패 클래스 6버킷**을 반드시 심문해 lock-in 결정으로.
...
각 T 에 **자동(유닛/CI) vs 수동(디바이스 e2e)** 표시 — 자동 가능분은 task 끝 검증에서 회귀 테스트로 남는다(pslog-workflow 6단계).

## 실패 클래스 6버킷 (렌즈 3·4 의 바닥 — 최소 이건 다 심문)
...
| 테스트 가능성 | 외부 의존(SDK/DB/API) 직결이면 seam(repository/port)은? 핵심 로직 순수 함수로 격리 가능? T1~Tn 중 유닛으로 돌 수 있는 건? |

**바닥 + 열린 심문**: 6버킷은 반드시 심문 + "이 설계만의 특수 리스크는?"(도메인 특수)도 묻는다.
